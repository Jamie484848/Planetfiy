...
<head>
    ...
    <title>Planetify - {{ _('Your Music') }}</title>
...
<body>
    ...
    <div class="app-container">
        ...
        <div class="main-content">
            ...
            <div class="header">
                ...
                <div class="language-switcher">
                    <select id="languageSelect" onchange="changeLanguage(this.value)">
                        <option value="de" {% if session.get('lang', 'de') == 'de' %}selected{% endif %}>🇩🇪 Deutsch</option>
                        <option value="en" {% if session.get('lang', 'de') == 'en' %}selected{% endif %}>🇬🇧 English</option>
                    </select>
                </div>
            ...
            <div class="section-title">{{ _('Your Music') }}</div>
    ...
    <div class="modal" id="uploadModal">
        <div class="modal-content">
            <div class="modal-header">{{ _('Upload Songs') }}</div>
            <div class="upload-area" id="uploadArea" onclick="document.getElementById('fileInput').click()">
                <input type="file" id="fileInput" accept=".mp3,.wav,.ogg,.flac,.m4a" multiple onchange="handleFileSelect(event)">
                <div class="upload-icon">📁</div>
                <p>{{ _('Click here or drag files in') }}</p>
                <p class="upload-info">{{ _('Supported: MP3, WAV, OGG, FLAC, M4A') }}</p>
            </div>
            <div class="file-list" id="fileList" style="display:none;"></div>
            <div class="modal-buttons">
                <button class="modal-btn modal-btn-cancel" onclick="closeUploadModal()">{{ _('Cancel') }}</button>
    ...
    <div class="modal" id="urlModal">
        <div class="modal-content">
            <div class="modal-header">{{ _('Add Song from URL') }}</div>
            <p style="color: #b3b3b3; margin-bottom: 20px;">
{{ _('Add a link from YouTube, Spotify or SoundCloud to automatically download the song!') }}
            </p>
            <div class="form-group">
                <label>{{ _('Enter URL') }}</label>
...
{{ _('Supported: YouTube, YouTube Music, Spotify, SoundCloud') }}
                </div>
            </div>
...
{{ _('⏳ Downloading...') }}
            ...
            <div class="modal-buttons">
                <button class="modal-btn modal-btn-cancel" onclick="closeUrlModal()">{{ _('Cancel') }}</button>
